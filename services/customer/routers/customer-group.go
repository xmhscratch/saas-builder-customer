...
// 		pageSize, err := models.Atoi(ginCtx.DefaultQuery("ps", "-1"))
// 		if err != nil {
// 			ctx.Error(err)(ginCtx)
// 			return
// 		}
...
// 		total := models.CountAllGroups(db, catFilter)
...
// 		ginCtx.JSON(http.StatusOK, group)
// 	}
// }
...
// FindAllCustomerGroups comment
func (ctx *RouteContext) FindAllCustomerGroups() gin.HandlerFunc {
	return func(ginCtx *gin.Context) {
		var (
		...
		results, err = models.FindAllCustomerGroupIDs(db, customerID)
		if err != nil {
			ctx.Error(err)(ginCtx)
			return
		}
